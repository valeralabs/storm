...
export default async function init() {
  ...
  log(
      "Storm is a secure authenticator and wallet available from your terminal"
  );
  ...
  if (response.value) {
    ...
    log("The Secret Key is stored securely in your operating system's keychain.")
    ...
    const response = await prompts({
      ...
      choices: [
        { title: 'Create a Key', description: "We'll create a new random Key for you", value: true },
        { title: 'Import a Key', description: "Use a Key you already have", value: false }
      ],
      initial: 1
    })
    ...
    if (response.shouldCreateKey){
      ...
      spinner.succeed('Created Secret Key');
      ...
      const { key, password } = await prompts([
        {
          ...
          validate: key => !bip39.validateMnemonic(key) ? 'Invalid Secret Key' : true
        },
        {
          type: 'password',
          name: 'password',
